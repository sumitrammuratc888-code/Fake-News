export function analyzeNews(text) {
  ...
  if (emotionalCount >= 2 || emotionalDensity > 0.04) {
  ...
  if (exclaimCount > 2) { score -= 15; flags.push("Excessive exclamation marks"); }
  ...
  if (trustedMention) { score += 10; }
  if (unreliableMention) { score -= 25; flags.push("References known unreliable source"); }

  const hasNumbers = /\d{1,}/.test(text);
  const hasQuotes = /[""\u201C\u201D]/.test(text);
  const hasProperNouns = /[A-Z][a-z]{2,}/.test(text.slice(1));
  if (!hasNumbers && !hasQuotes && !hasProperNouns && wordCount > 15) {
    score -= 10;
  ...
  if (absolutistCount >= 1) {
    score -= absolutistCount * 5;
  ...
  if (urlCount > 2) { score -= 10; flags.push("Contains multiple external links"); }
  ...
  if (score >= 70) {
    ...
    reason = flags.length === 0
      ...
      : "The text appears mostly credible but has minor concerns worth noting.";
    suggestion = "Still, cross-check with trusted outlets like Reuters, AP News, or BBC for absolute confirmation.";
  } else if (score >= 45) {
    verdict = "Misleading";
    confidence = (50 + Math.floor((70 - score) * 1.5)) + "%";
    ...
    verdict = "Fake";
    confidence = Math.min(99, 60 + Math.floor((45 - score) * 1.5)) + "%";
